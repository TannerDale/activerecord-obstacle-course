...
# ----------------------
  ...
  it '24. returns all orders for user 2 which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    orders = Order.joins(:order_items).where('orders.user_id = ? AND order_items.item_id = ?', @user_2.id, @item_4.id)
    # -----------------------------------------------------------
  ...
  it '25. returns items that are associated with one or more orders' do
    ...
    ordered_items = Item.joins(:order_items).where('order_items.item_id = items.id').distinct.order(:name)
    # ---------------------------------------------------------------
